...
(
  ...
  logic mulv_en;
  logic [3:0] mulv_lane_idx;
  assign mulv_lane_idx = insn[31:28];
  ...
  mulv_type_t mulv_vector_type_bignum;
  assign mulv_vector_type_bignum = mulv_type_t'(insn[27:25]);
  ...
  assign insn_dec_bignum_o = '{
    ...
    alu_vector_type:     alu_vector_type_bignum,
    alu_vector_sel:      alu_vector_sel_bignum,
    ...
    mulv_en:             mulv_en,
    mulv_lane_idx:       mulv_lane_idx,
    mulv_type:           mulv_vector_type_bignum,
    rf_we:               rf_we_bignum,
  ...
  always_comb begin
    ...
    // TODO: remove from this decoder and refactor all instructions
    // s.t. bits 27:26 are always the vector type (BN.SHV not yet,
    // since 25:29 is shift amount)
	...
	alu_vector_type_bignum = alu_vector_type_t'({2'b01, insn[16]});
      ...
      InsnOpcodeBignumMulv: begin
        ...
        rf_ren_a_bignum     = 1'b1;
        rf_ren_b_bignum     = 1'b1;
        rf_wdata_sel_bignum = RfWdSelMulv;
        mulv_en       = 1'b1;
        rf_we_bignum = 1'b1;
      end
